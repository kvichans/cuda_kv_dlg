...
pass;                           LOG_FREE    = 0         # No order (0 as False)
pass;                           LOG_ALLOW   = 1         # Allowed  (1 as True)
pass;                           LOG_NEED    = 2         # Required
pass;                           LOG_FORBID  = 3         # Forbidden
pass;                           log4mod     = LOG_FREE  # Order log in the module
def iflog(*logs):
    if      any([LOG_FORBID  ==l for l in logs]): return False  # if at least one is FORBID
    if      any([LOG_NEED    ==l for l in logs]): return True   # if at least one is NEED 
    return  any([LOG_ALLOW   ==l for l in logs])                # if at least one is ALLOW
    
REDUCTIONS  = {'lbel'   :'label'
            ,  'lilb'   :'linklabel'
            ,  'edit'   :'edit'
            ,  'edtp'   :'edit_pwd'
            ,  'sped'   :'spinedit'
...
_fit_top_by_env__cash    = {}
def _fit_top_by_env__clear():
    global _fit_top_by_env__cash
    _fit_top_by_env__cash    = {}
def _fit_top_by_env(what_tp, base_tp='label'):
    ...
    if what_tp==base_tp:
        return 0
    if (what_tp, base_tp) in _fit_top_by_env__cash:
        pass;                  #log('cashed what_tp, base_tp={}',(what_tp, base_tp))
        return _fit_top_by_env__cash[(what_tp, base_tp)]
    ...
    return _fit_top_by_env__cash.setdefault((what_tp, base_tp), fit)
   #def _fit_top_by_env
...
def _dlg_proc_wpr(id_dialog, id_action, prop='', index=-1, index2=-1, name=''):
    ...
    if id_action==app.DLG_SCALE:
        return
#   if id_dialog==0:
#       print('idd=dlg_proc(0, {})'.format(_DLG_PROC_I2S[id_action]))
#   else:
#       print('dlg_proc(idd, {}, index={}, name="{}", prop={}, index2={})'.format(   _DLG_PROC_I2S[id_action], 
#           index, name, 
...
def _form_acts(act, fprs=None, did=None, key4store=None):
    ...
    pass;                       log('act, fprs, did={}',(act, fprs, did)) if iflog(log4fun,log4mod) else 0
    ...
    fprs    = _dlg_proc_wpr(did, app.DLG_PROP_GET)  if act=='save' and did else fprs
    fm_key  = key4store if key4store else gen_form_key(fprs)
    pass;                       log('fm_key, fprs={}',(fm_key, fprs)) if iflog(log4fun,log4mod) else 0
    if False:pass
    elif act=='move' and fprs:
        prev    = get_hist(fm_key)
        pass;                   log('prev={}',(prev)) if iflog(log4fun,log4mod) else 0
        if not prev:    return fprs
        if not fprs.get('resize', False):
            prev.pop('w', None)
            prev.pop('h', None)
        fprs.update(prev)
        pass;                   log('!upd fprs={}',(fprs)) if iflog(log4fun,log4mod) else 0
        return fprs
    elif act=='save' and did:
        set_hist(fm_key, {k:v for k,v in fprs.items() if k in ('x','y','w','h')})
   #def _form_acts

CB_HIDE   = lambda cid,ag,d='':None     # Callback to hide dlg

class DlgAg:
    def __init__(self, ctrls, form=None, fid=None, opts=None):
        """ Create dialog
        """
        pass;                   log4fun=-1==-1  # Order log in the function
        # Fields
    ...
    def show(self, callbk_on_exit=None):
        ...
        if callbk_on_exit and callable(callbk_on_exit):  callbk_on_exit(self)
#       self._before_free()
        _dlg_proc_wpr(self.did, app.DLG_FREE)
...
#       return self._take_val(name, rsp, defv)   if attr=='val' and live else    rsp
    ...
    def update(self, upds):
        """ Update most of dlg mem-data
            upds is {vals={cid:v}, ctrls=[(cid,{k:v})], form={k:v}, fid:cid}
                 or list of such data
        """
        pass;                   log4fun=-1==-1  # Order log in the function
        ...
        if 'fid' in upds and upds['fid'] in self.ctrls:
    ...
    def _setup(self, ctrls, form, fid=None):
        ...
        for cid, ccfg in ctrls:
            ...
            pass;              #ccfg['_idc']    = ind_c         # While API bug: name isnot work if contorl is in panel
            pass;              #log('ind_c,ccfg[type]={}',(ind_c,ccfg['type']))
           #for cnt
...
#           return {k:v for (k,v) in cnt.items() if k in ('x','y','w','h')}
        ...
        and cnt['type'] in (  'button', 'checkbutton'
            ...
            cnt['h']    = _get_gui_height(cnt['type'])
            prP['_ready_h'] = True
#           cnt['h']    = app.app_proc(app.PROC_GET_GUI_HEIGHT, cnt['type'])
            pass;              #log('cnt={}',(cnt)) if cnt['type']=='checkbutton' else 0
...
#       t       = cnt.get('t', 0)   if 't' in cnt else  self.cattr(cid, 't', live=False)
            ...
            pass;              #log('tp, bs_tp, fit, bs_cnt={}',(tp, bs_tp, _fit_top_by_env(tp, bs_tp), bs_cnt))
            t       = bs_cnt4t['t'] + _fit_top_by_env(tp, bs_tp)
            prP['y']    = t
...
#                         ,ma=int( ci[3])
    ...
    def _preprocessor(cnt, tp):
        pass;                   log4fun=-1== 1  # Order log in the function
        pass;                   log('tp,cnt={}',(tp,cnt)) if iflog(log4fun,log4mod) else 0
        # call -> on_???
        if 'call' in cnt:
            if False:pass
            elif tp in ('listview', 'treeview'):
                cnt['on_select']    = cnt['call']
            elif tp in ('linklabel'):
                cnt['on_click']     = cnt['call']
            else:
                cnt['on_change']    = cnt['call']
        ...
        if 'props' in cnt and app.app_api_version()>='1.0.224':
            ...
            pass;               log('lsPr={}',(lsPr)) if iflog(log4fun,log4mod) else 0
            if False:pass
            elif tp=='button':
        ...
        pass;                   log('cnt={}',(cnt)) if iflog(log4fun,log4mod) else 0
       #def _preprocessor
    ...
    def _gen_repro_code(self):
        ...
        for idC in range(app.dlg_proc(self.did, app.DLG_CTL_COUNT)):
            prC = _dlg_proc_wpr(self.did, app.DLG_CTL_PROP_GET, index=idC)
            cids+=[prC['name']]
            ...
            srp+=l
        prD     = _dlg_proc_wpr(self.did, app.DLG_PROP_GET)
...
#       app.dlg_proc(self.id_dlg, app.DLG_SCALE)        #??
        ...
        self._before_free()
        _dlg_proc_wpr(self.id_dlg, app.DLG_FREE)
    ...
    def fattr(self, attr, live=True, defv=None):
        """ Return one form property """
        attr= 'focused' if attr=='fid' else attr
        pr  = _dlg_proc_wpr(self.id_dlg
                        , app.DLG_PROP_GET)     if live else    self.form
        pass;                  #log('pr={}',(pr))
        rsp = pr.get(attr, defv)
        if live and attr=='focused':
            prf = _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_GET, index=rsp)
            rsp = prf['name'] if prf else None
        return rsp
       #def fattr
...
#       return self._take_val(name, rsp, defv)   if attr=='val' and live else    rsp
    ...
    def __init__(self, ctrls, form=None, focused=None, options=None):
        ...
        self.id_dlg = _dlg_proc_wpr(0, app.DLG_CREATE)
...
#               raise Exception(f('Unknown focused: {}', focused))
        ...
        for name, cfg_ctrl in ctrls:
            ...
            ind_c   = _dlg_proc_wpr(self.id_dlg
                        , DLG_CTL_ADD_SET
                        , name=cfg_ctrl['type']
...
#               pr  = {k:(lambda idd,idc,data:print(repr(k))) for k in pr}
        ...
        for idC in range(app.dlg_proc(self.id_dlg, app.DLG_CTL_COUNT)):
            prC = _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_GET, index=idC)
            if ''==prC.get('hint', ''):                 prC.pop('hint', None)
            if ''==prC.get('tag', ''):                  prC.pop('tag', None)
            if ''==prC.get('cap', ''):                  prC.pop('cap', None)
            if ''==prC.get('items', None):              prC.pop('items')
        ...
        prD     = _dlg_proc_wpr(self.id_dlg, app.DLG_PROP_GET)
        prD.update(self.form)
        srp    +=l+f('dlg_proc(idd, DLG_PROP_SET, prop={})', out_attrs(prD, fattrs))
    ...
    def _form_acts(act, form=None, id_dlg=None, key4store=None):
        ...
        if act=='save' and id_dlg:
            dlg_pr  = _dlg_proc_wpr(id_dlg, app.DLG_PROP_GET)
    ...
    def _prepare_anchors(self):
        ...
        for cid,cnt in self.ctrls.items():
            ...
            if aid in self.ctrls:
                prTrg   = _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_GET, name=aid)
                trg_w,  \
                trg_h   = prTrg['w'], prTrg['h']
            prOld   = _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_GET, name=cid)
#           if not prOld:
#               # While API bug: name isnot work
#               idc     = self.ctrls[cid]['_idc']
#               prOld   = _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_GET, index=idc)
#               pass;           log('fail CTL_PROP_GET cid,idc,prOld={}',(cid,idc,prOld))
#               prOld   = cnt
#               if not prOld:
#                   continue#for cid,cnt
#           pass;               prOld   = prOld if prOld else \
#                     _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_GET, index=self.ctrls[cid]['_idc'])    # While API bug: name isnot work if contorl is in panel
            ...
            if prAnc:
                ...
                cnt.update(prAnc)
                _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_SET, name=cid, prop=prAnc)
#               pass;           pr_   = _dlg_proc_wpr(self.id_dlg, app.DLG_CTL_PROP_GET, name=cid)
...
if __name__ == '__main__' :
    ...
    if -1==-1:
        print('Start test1: dlg: (label, edit, button), (call, update, hide, on_exit)')
        ...
        DlgAg(
            ctrls=[0
   ,('l1', dict(tp='lbel'   ,cap='>l&abel'      ,x=0   ,tid='e1'   ,w= 50))
   ,('e1', dict(tp='edit'   ,val='Edit me'      ,x=50  ,t=  0      ,w=150))
   ,('b1', dict(tp='bttn'   ,cap='Re&name me'   ,x=0   ,y= 30      ,w=200  ,call=do_call))
   ,('b2', dict(tp='bttn'   ,cap='Sh&ort me'    ,x=0   ,y= 60      ,w=200  ,call=do_call   ,def_bttn=1))
   ,('b3', dict(tp='bttn'   ,cap='A&sk,Close'   ,x=0   ,y= 90      ,w=200  ,call=do_call))
   ,('cl', dict(tp='bttn'   ,cap='Close'        ,x=0   ,y=120      ,w=200  ,call=CB_HIDE))
   ][1:]
        ,   form=dict(cap='Try1'                  ,h=150,w=200)
#       ,   fid='b1'
#                               ,opts=dict(gen_repro_to_file='repro_try1.py')
        ).show(lambda ag: log('on hide'))
        print('Stop test1')
